...
- **`src/telegram_bot.py`**:
  ...
  - **Interceptação Inteligente de Menus (`check_main_menu_intercept`)**: Garante que o clique em qualquer botão do menu fixo inferior encerre instantaneamente conversas pendentes anteriores (ex: Postagem Simultânea, YouTube, Dailymotion, Busca TMDB), redirecionando imediatamente para a função acionada e impedindo que o texto do botão seja interpretado erroneamente como nome de filme.
